...
class MapIISpec extends FlatSpec with Matchers {
  ...
  """We'll say that 2 strings "match" if they are non-empty and their first chars are the same. Method "firstSwap"""" should """loop over and then return the given array of non-empty strings as follows: if a string matches an earlier string in the array, swap the 2 strings in the array. A particular first char can only cause 1 swap, so once a char has caused a swap, its later swaps are disabled.""" in {
    MapII.firstSwap(List("ab", "ac")) should equal(List("ac", "ab"))
    MapII.firstSwap(List("ax", "bx", "cx", "cy", "by", "ay", "aaa", "azz")) should equal(List("ay", "by", "cy", "cx", "bx", "ax", "aaa", "azz"))
    MapII.firstSwap(List("ax", "bx", "ay", "by", "ai", "aj", "bx", "by")) should equal(List("ay", "by", "ax", "bx", "ai", "aj", "bx", "by"))
  }
